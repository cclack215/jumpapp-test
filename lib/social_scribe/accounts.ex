defmodule SocialScribe.Accounts do
  ...
  @doc """
  Finds or creates a user credential from a map (for HubSpot and other custom OAuth flows).
  """
  def find_or_create_user_credential(user, attrs) when is_map(attrs) do
    provider = Map.get(attrs, :provider) || Map.get(attrs, "provider")
    uid = Map.get(attrs, :uid) || Map.get(attrs, "uid")

    case get_user_credential(user, provider, uid) do
      nil ->
