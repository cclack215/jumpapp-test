defmodule SocialScribe.CalendarSyncronizer do
  ...
  defp fetch_and_sync_for_credential(%UserCredential{} = credential) do
    with {:ok, token} <- ensure_valid_token(credential),
      ...
      {:error, {:refresh_failed, {_status, %{"error" => "missing_refresh_token"} = error_data}}} ->
        # Expected error - user needs to reconnect. Log as warning, not error.
        Logger.warn("Skipping sync for credential #{credential.id}: refresh_token missing. User should reconnect their Google account.")
        {:error, {:refresh_failed, error_data}}

      {:error, {403, %{"error" => %{"errors" => [%{"reason" => "accessNotConfigured"} | _]}} = error_data}} ->
        # Google Calendar API not enabled - configuration issue
        activation_url = get_in(error_data, ["error", "details", Access.at(0), "metadata", "activationUrl"]) ||
                         "https://console.developers.google.com/apis/library/calendar-json.googleapis.com"
        Logger.warn(
          "Skipping sync for credential #{credential.id}: Google Calendar API is not enabled. " <>
          "Please enable it in Google Cloud Console: #{activation_url}"
        )
        {:error, {:api_not_enabled, error_data}}
  ...
  defp ensure_valid_token(%UserCredential{} = credential) do
    if DateTime.compare(credential.expires_at || DateTime.utc_now(), DateTime.utc_now()) == :lt do
      ...
      if is_nil(credential.refresh_token) || credential.refresh_token == "" do
        Logger.warn("Cannot refresh token for credential #{credential.id}: refresh_token is missing. User needs to re-authenticate.")
        {:error, {:refresh_failed, {400, %{"error" => "missing_refresh_token", "error_description" => "Refresh token is missing. Please reconnect your account."}}}}
      else
