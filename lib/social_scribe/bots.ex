defmodule SocialScribe.Bots do
  ...
  def create_and_dispatch_bot(user, calendar_event) do
    ...
    with {:ok, %{status: status, body: api_response}} when status in 200..299 <-
      ...
      initial_status = extract_status_code(api_response, :first)
  ...
  def update_bot_schedule(bot, calendar_event) do
    ...
    with {:ok, %{body: api_response}} <-
      ...
      new_status = extract_status_code(api_response, :first)
  ...
  defp extract_status_code(api_response, which) do
    case Map.get(api_response, :status_changes) do
      list when is_list(list) and list != [] ->
        ...
        Map.get(entry, :code, "pending")

      _ ->
        Map.get(api_response, :status, "pending")
    end
  end
