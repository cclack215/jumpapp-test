defmodule SocialScribe.Meetings do
  ...
  def create_meeting_from_recall_data(%RecallBot{} = recall_bot, bot_api_info, transcript_data) do
    ...
    Repo.transaction(fn ->
      ...
      transcript_attrs = parse_transcript_attrs(meeting, normalized_transcript_data)

      {:ok, _transcript} = create_meeting_transcript(transcript_attrs)

      participants = resolve_participants(bot_api_info, normalized_transcript_data)
      ...
      Enum.each(participants, fn participant_data ->
        case participant_data do
          %{id: _id, name: _name} ->
            ...
            _ = create_meeting_participant(participant_attrs)
            :ok

          other ->
            Logger.warn("Skipping meeting participant with unexpected format: #{inspect(other)}")
        end
      end)
  ...
  defp normalize_transcript_data(transcript_data) when is_binary(transcript_data) do
    case Jason.decode(transcript_data) do
      {:ok, decoded} -> decoded
      _ -> transcript_data
    end
  end

  defp normalize_transcript_data(transcript_data), do: transcript_data

  defp resolve_participants(bot_api_info, transcript_data) do
    bot_participants =
      Map.get(bot_api_info, :meeting_participants) ||
        Map.get(bot_api_info, "meeting_participants") ||
        []

    case bot_participants do
      list when is_list(list) and list != [] ->
        list

      _ ->
        derive_participants_from_transcript(transcript_data)
    end
  end

  defp derive_participants_from_transcript(transcript_data) when is_list(transcript_data) do
    transcript_data
    |> Enum.reduce(%{}, fn segment, acc ->
      case extract_participant_from_segment(segment) do
        nil ->
          acc

        %{id: id, name: name} = participant ->
          key =
            cond do
              not is_nil(id) -> {:id, id}
          ...
          case key do
            :ignore -> acc
            _ -> Map.put_new(acc, key, participant)
          end
      end
    end)
    |> Map.values()
  ...
  defp derive_participants_from_transcript(_), do: []

  defp extract_participant_from_segment(%{} = segment) do
    participant =
    ...
    cond do
      is_map(participant) ->
        id = Map.get(participant, :id) || Map.get(participant, "id")
        name = Map.get(participant, :name) || Map.get(participant, "name")
        is_host =
          Map.get(participant, :is_host) ||
            Map.get(participant, "is_host") || false

        %{id: id, name: name, is_host: is_host}

      true ->
        name = Map.get(segment, :speaker) || Map.get(segment, "speaker")
        id = Map.get(segment, :speaker_id) || Map.get(segment, "speaker_id")

        if is_nil(name) and is_nil(id) do
          nil
        else
          %{id: id, name: name, is_host: false}
        end
    end
  end

  defp extract_participant_from_segment(_), do: nil

  defp parse_meeting_attrs(calendar_event, recall_bot, bot_api_info) do
    recording_info = List.first(bot_api_info.recordings || []) || %{}

    completed_at =
      case recording_info do
        %{completed_at: completed} when is_binary(completed) ->
    ...
    raw_recorded_at =
      case recording_info do
        %{started_at: started} when is_binary(started) ->
    ...
    # Fallback to the calendar event's start_time so recorded_at is never nil
    recorded_at = raw_recorded_at || calendar_event.start_time
    ...
    language =
      case transcript_data do
        [first | _] when is_map(first) ->
          Map.get(first, :language) || Map.get(first, "language") || "unknown"
  ...
  defp parse_participant_attrs(meeting, participant_data) do
    id = Map.get(participant_data, :id)
    name = Map.get(participant_data, :name)

    %{
      meeting_id: meeting.id,
      recall_participant_id: (id && to_string(id)) || "",
      name: name || "",
  ...
  defp format_transcript_for_prompt(transcript_segments) when is_list(transcript_segments) do
    Enum.map_join(transcript_segments, "\n", fn segment ->
      ...
      # 1) Simple segments with a "text" field (what Recall.ai returns in your JSON)
      # 2) Detailed segments with a "words" list (original implementation)
      text =
        cond do
          is_binary(Map.get(segment, "text")) and String.trim(Map.get(segment, "text")) != "" ->
            Map.get(segment, "text")

          is_list(Map.get(segment, "words")) ->
            Enum.map_join(Map.get(segment, "words", []), " ", &Map.get(&1, "text", ""))
