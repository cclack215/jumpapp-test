defmodule SocialScribeWeb.HomeLive do
  ...
  def handle_info({:schedule_bot, event}, socket) do
    if event.record_meeting do
      case Bots.create_and_dispatch_bot(socket.assigns.current_user, event) do
        ...
        {:error, reason} ->
          Logger.error("Failed to create and dispatch bot for event #{event.id}: #{inspect(reason)}")
          # Optionally show a flash message to the user
          send(self(), {:bot_error, "Failed to schedule bot. Please try again."})
      end
    else
  ...
  @impl true
  def handle_info({:bot_error, message}, socket) do
